...
- ### The beta-binomial model
	...
	- #### Bernoulli likelihood
	    ...
	    - 각 시행의 결과를 나타내는 변수 $y_n$​은 베르누이 분포를 따릅니다.
		    - 각 시행의 확률을 나타내는 파라미터는 $θ$로 표현됩니다.
		    - 데이터셋 $D$에서 나온 모든 결과를 곱한 우도 함수는 다음과 같습니다.
			
			$p(D|\theta)=\Pi_{n=1}^{N}​θ^{y_n}​(1−θ)^{1−y_n}$​
			
			- 여기서 $N$은 전체 시행 횟수이고, $N_1$​은 앞면이 나온 횟수, $N_0$​은 뒷면이 나온 횟수입니다.
			- 이는 다음과 같이 표현할 수 있습니다.
			
			$p(D∣θ)=θN1​(1−θ)N0​$
			
			$N=N_0​+N_1​$은 전체 샘플 크기로, 모든 시행의 횟수를 나타냅니다.
		
	- #### Binomial Likelihood
		 - 이번에는 N번의 독립적인 동전 던지기 시행에서 앞면이 나오는 횟수를 나타내는 변수를 고려합니다.
		    이항 분포의 우도 함수는 다음과 같습니다.
			
			$p(D|θ)=\Pi_{n=1}^{N}​\begin{pmatrix}N​\\​y_n\end{pmatrix}θ^{y_n}​(1−θ)^{N−y_n}​$
			
			여기서 ​$\begin{pmatrix}N​\\​y_n\end{pmatrix}$은 N번 중에서 $y_n$​번만큼 성공(앞면)이 나올 조합의 수를 나타냅니다.
			우도 함수는 베르누이 분포의 우도와 유사하게 표현됩니다.
		    이항 분포 우도는 베르누이 분포의 우도와 스케일링 요소를 제외하면 동일한 형태를 가집니다.
		    
			$p(D∣θ)=θ^{N_1}​(1−θ)^{N_0}​$
			
			여기서 $N_1$​은 성공(앞면) 횟수, �0N0​은 실패(뒷면) 횟수입니다.
			
			이렇게 두 경우 모두에서, 각 시행의 결과가 독립적이고 동일한 분포를 따른다는 가정 하에 우도 함수가 정의되었습니다. 이를 통해 앞면이 나오는 횟수와 같은 관측치를 가지고 주어진 파라미터 $θ$에 대한 추정을 수행할 수 있습니다.
