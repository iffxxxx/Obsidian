...
- ### The metod of moments
	- 모멘트 방법(Method of Moments, MOM)은 통계 분포의 매개변수를 추정하기 위한 최대우도추정(MLE)의 대안입니다. MOM은 분포의 이론적 모멘트를 경험적 모멘트에 대입하고 이로부터 얻은 연립 방정식을 푸는 방법입니다.
		...
		예: 단일 변수 가우시안의 MOM 단일 변수 가우시안 분포를 고려해 봅시다. 이론적 모멘트는 µ1 = µ(평균) 및 µ2 = σ^2 + µ^2(분산)입니다. 경험적 모멘트는 데이터에서 계산되며 평균 및 분산의 MOM 추정치는 각각 µˆ = y 및 σˆ^2 = s^2 - y^2입니다.
	
		예: 균등 분포의 MOM 균등 분포 Unif(θ1, θ2)의 경우 첫 번째 두 모멘트를 사용하여 추정합니다. (θ1, θ2) 매개변수는 이론적 및 경험적 모멘트를 활용하여 방정식을 풀어 얻습니다. 그러나 특정 상황에서 MOM은 부적절한 결과를 낼 수 있습니다.
