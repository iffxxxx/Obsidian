...
- ### Surrogate loss
	...
	- **Hinge Loss:**
		...
		힌지 손실은 0-1 손실의 볼록한 상한을 제공하지만, 미분가능하지 않은 부분이 있습니다.
...
- ### The metod of moments
	- 모멘트 방법(Method of Moments, MOM)은 통계 분포의 매개변수를 추정하기 위한 최대우도추정(MLE)의 대안입니다. MOM은 분포의 이론적 모멘트를 경험적 모멘트에 대입하고 이로부터 얻은 연립 방정식을 푸는 방법입니다.
		...
		이론적 모멘트(µk)는 랜덤 변수 Y의 특정 함수들의 기대값을 나타냅니다. 예를 들어, $\mu1 = E[Y], \mu2 = E[Y^2]$ 등입니다. 경험적 모멘트는 데이터에서 추정되며 $\mu^k$로 표기됩니다. 예를 들면 µˆk = (1/N)Σn=1부터 N까지 yn^k이고, 여기서 N은 표본 크기입니다.
...
- ### Online (recursive) estimation
	- 순차적으로 도착하는 데이터를 처리할 때 온라인 학습 또는 재귀 추정이 사용됩니다. 새로운 데이터 포인트가 이용 가능해짐에 따라 매개 변수 추정치를 업데이트하는 개념입니다. 재귀 업데이트는 θt = f(θˆt−1, yt)와 같은 형태를 가지며 여기서 θt는 업데이트된 매개 변수, θˆt−1은 이전 추정치, yt는 새로운 데이터 포인트입니다.
	
	- 예: 가우시안 평균의 재귀 MLE 가우시안 분포의 평균에 대한 재귀 추정을 고려해 봅시다. 일괄 처리 추정 µˆt = (1/t)Σn=1부터 t까지 yn을 사용하여 µˆt = µˆt−1 + (1/t)(yt - µˆt−1)로 재귀적으로 업데이트할 수 있습니다. 이를 이동 평균이라고 하며 이를 통해 추정치가 새로운 데이터에 적응하면서 이전 데이터의 영향이 줄어듭니다.
	
	- 지수 가중 이동 평균(EWMA): 온라인 학습에서 최근 예제에 더 많은 가중치를 부여하기 위해 지수 가중 이동 평균(EWMA)가 도입됩니다. 업데이트 규칙은 µˆt = βµt−1 + (1 − β)yt, 여기서 0 < β < 1입니다. EWMA는 새로운 데이터에 대한 적응성과 추정치의 부드러움 사이의 균형을 제공합니다. 초기 편향을 완화하기 위해 경우에 따라 편향 수정을 적용할 수 있습니다.
