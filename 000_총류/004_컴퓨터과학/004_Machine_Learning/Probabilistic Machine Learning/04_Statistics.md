...
- ### Surrogate loss
	...
	-  **Log Loss:**
		...
		$p(\tilde{y}|x,\theta)=\sigma(\tilde{y}​\eta)=\frac{1}{1+e^{-\tilde{y}η}}​$
		...
		- **Log-odds?**
			 ...
			 이를 수식으로 나타내면 다음과 같습니다:
			  ...
			  $p(y=1|x)$ 확률 값이 다음 그래프와 같이 1에 가까워질수록 odds 값은 엄청나게 상승
	...
	- **Hinge Loss:**
		...
		- **Convex Function and Upper Bound**:
			**볼록 함수(Convex Function):**  함수 그래프 상의 임의의 두 점을 잇는 선분이 그래프 상의 지점들보다 항상 위에 위치하는 것을 의미합니다.
			
			**함수의 상한(Upper Bound):** 어떤 함수 $g(x)$가 함수 $f(x)$의 상한이라면 다음이 성립합니다.
			$g(x)≥f(x),∀x$
...
- ### Online (recursive) estimation
	...
	- 지수 가중 이동 평균(EWMA): 온라인 학습에서 최근 예제에 더 많은 가중치를 부여하기 위해 지수 가중 이동 평균(EWMA)가 도입됩니다. 업데이트 규칙은 다음과 같다.
		...
		- $\eta_{t} = \beta\eta_{t−1} + (1 -\beta)y_{t}$, 
			- $\mu_t$는 현재 시점 $t$에서의 EWMA 추정치입니다.
			- $\mu_{t−1}$​은 이전 시점 $t−1$에서의 EWMA 추정치입니다.
			- $\beta$는 0과 1 사이의 상수이며, $\beta$에 더 가까울수록 이전 추정치에 더 많은 가중치가 부여됩니다.
			- $y_t$​는 현재 시점 $t$에서의 새로운 데이터 포인트입니다.
			  
		여기서 0 < β < 1입니다. 0 < β < 1이므로, 우리는 βt+1 → 0은 t → ∞이므로, β가 작을수록 과거를 더 빨리 잊고 최근 데이터에 보다 최근 데이터에 더 빠르게 적응합니다. 이는 그림에 설명되어 있습니다. 
...
3. **정규화된 목적 함수:**
    ...
    - 과적합 문제를 해결하기 위한 주요 방법 중 하나는 **정규화**(Regularization)를 사용하는 것입니다. 정규화는 음의 로그 우도 또는 경험적 위험에 패널티 항을 추가하여 목적 함수를 최적화하는 것을 의미합니다. 목적 함수는 다음과 같이 정의됩니다.
		...
		여기서 �λ는 정규화 매개변수이고, �(�)C(θ)는 복잡성 페널티입니다.
1. **복잡성 패널티:**
    
    - 일반적인 복잡성 패널티는 �(�)=−log⁡�(�)C(θ)=−logp(θ)로, 여기서 �(�)p(θ)는 모델 매개변수에 대한 사전 확률입니다.
    - 정규화된 목적은 �(�;�)=−1�∑�=1�log⁡�(��∣��,�)−�log⁡�(�)L(θ;λ)=−N1​∑n=1N​logp(yn​∣xn​,θ)−λlogp(θ)로 됩니다.
5. **MAP 추정:**
    
    - �=1λ=1로 설정하고 �(�)p(θ)를 적절하게 재조정하면 정규화된 목적은 로그 사후분포를 최대화하는 것과 동일합니다: �MAP=arg⁡max⁡�log⁡�(�∣�)θMAP​=argmaxθ​logp(θ∣D).
    - MAP 추정은 최대 사후 추정을 나타내며 베이지안 관점을 통합합니다.
6. **예제: 베르누이 분포에 대한 MAP 추정:**
    
    - 동전 던지기의 맥락에서 MAP 추정은 과적합을 피하기 위해 적용됩니다. 베타 분포가 사전으로 사용되며 이로부터 파생된 MAP 추정이 제시됩니다.
7. **예제: 다변량 가우시안에 대한 MAP 추정:**
    
    - 다변량 가우시안의 평균과 공분산의 MLE 추정에 MAP 추정을 적용합니다. 역 위샤트 사전이 공분산 행렬에 사용되며 수축 추정이 얻어집니다.
8. **예제: 가중치 감쇠 (L2 정규화):**
    
    - L2 정규화 또는 가중치 감쇠는 손실에 페널티 항을 추가하는 것으로, 과적합을 방지하는 데 도움
