...
- ### The beta-binomial model
	...
	- #### Binomial Likelihood
		 - 이번에는 N번의 독립적인 동전 던지기 시행에서 앞면이 나오는 횟수를 나타내는 변수를 고려합니다.
			...
			$p(D|θ)=\Pi_{n=1}^{N}​(y_n​N​)θyn​(1−θ)N−yn​$
		
		- 여기서 (���)(yn​N​)은 N번 중에서 ��yn​번만큼 성공(앞면)이 나올 조합의 수를 나타냅니다.
...
�(�∣�)=��1(1−�)�0p(D∣θ)=θN1​(1−θ)N0​

- 여기서 �1N1​은 성공(앞면) 횟수, �0N0​은 실패(뒷면) 횟수입니다.

이렇게 두 경우 모두에서, 각 시행의 결과가 독립적이고 동일한 분포를 따른다는 가정 하에 우도 함수가 정의되었습니다. 이를 통해 앞면이 나오는 횟수와 같은 관측치를 가지고 주어진 파라미터 �θ에 대한 추정을 수행할 수 있습니다.
