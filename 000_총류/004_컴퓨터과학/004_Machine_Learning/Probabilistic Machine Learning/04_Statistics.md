...
- ### The beta-binomial model
	- **개요:**
		...
		- 각 동전 던지기의 결과를 $y_n$으로 나타냅니다.
	    - $y_n = 1$은 n번째 시행이 앞면이 나온 사건을 나타내고, $y_n = 0$은 뒷면이 나온 사건을 나타냅니다.
	    - 모든 데이터를 $D = {y_n : n = 1 : N}$로 표기하며, N번의 시행 결과를 모두 담고 있습니다.
	    - 각 시행의 결과 $y_n$은 베르누이 분포를 따르며, 확률 θ로 정의됩니다. 여기서 θ는 $[0, 1]$ 구간에 속하는 확률로, 동전이 앞면이 나올 확률을 나타냅니다.
	...
	2. **베이지안 업데이트**:
	    ...
	    - 사전 분포(prior)로서 베타 분포를 선택합니다. 베타 분포는 [0, 1] 구간에서 연속적인 값을 가지며, 사전 지식이 없는 경우에도 적절하게 사용될 수 있는 분포입니다.
	    - 베타 분포의 매개변수는 α와 β로 표현되며, 이를 통해 사전적인 믿음을 표현할 수 있습니다.
	    - 사전 분포와 데이터에 기반하여 베타 분포의 매개변수를 업데이트합니다.
	3. **사후 분포 계산**:
	    
	    - 베이즈 정리를 사용하여 사후 분포 p(θ|D)를 계산합니다.
	    - p(θ|D) ∝ p(D|θ) * p(θ), 여기서 p(D|θ)는 우도(likelihood)를, p(θ)는 베타 분포를 나타냅니다.
	    - 이 사후 분포는 사후적으로 얻은 확률 θ에 대한 추정치를 제공합니다.
	4. **추론**:
	    
	    - 계산된 사후 분포를 통해 추론을 수행할 수 있습니다. 이를 통해 동전이 앞면이 나올 확률에 대한 정보를 더 정확하게 알 수 있습니다.

이렇게 베타-이항 모델은 동전 던지기와 같이 이항 분포를 갖는 문제에서 베이지안 추론을 수행하는데 유용하게 사용됩니다.
