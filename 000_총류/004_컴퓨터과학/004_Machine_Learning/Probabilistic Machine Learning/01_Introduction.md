...
- ### Clustering
	...
	- **방법:**
		1. 통계학계에서는 모델링되지 않은 외생 변수를 나타내기 위해 x를 사용하는 것이 일반적이지만, 단순히 입력으로 주어진 변수입니다. 따라서 무조건적인 모델은 p(x)가 아닌 p(y)로 표시됩니다.
...
- ### Discovering latent "factors of variation"
	...
	- **FA모델:**
		  기본적으로는 각 잠재적인 요소 $z_n$이 가우시안과 같은 간단한 확률 모델을 따르는 것으로 가정합니다. 가우시안이라면 각 요소가 무작위 K차원 벡터인 것으로 간주됩니다. 선형 모델을 사용하는 경우, $p(x_n|z_n; θ) = N (x_n|Wz_n + µ, Σ)$로 표현할 수 있으며, 이를 통해 Factor Analysis (FA)라는 모델을 얻을 수 있습니다.
...
- ### Preprocessing text data
	...
	- #### Bag of Words model(BoW)
		...
		- **수식:**
			  문서 $n$에서 위치 $t$의 토큰을 $x_{nt}$로 표기하겠습니다. 어휘에는 $D$개의 고유한 토큰이 있다면, $n$번째 문서를 $\tilde{x}_{n}$이라는 $D$차원 벡터로 나타낼 수 있습니다. 여기서 $\tilde{x}_{nv}$는 단어 $v$가 문서 $n$에서 나타나는 횟수입니다:
			
			$\tilde{x}_{nv}=\sum_{t=1}^{T}​I(x_{nt}=v)$
			
			여기서 T는 문서 n의 길이입니다. 이제 문서를 $R^D$의 벡터로 해석할 수 있습니다. 이것이 텍스트의 벡터 공간 모델(vector space model)이라고 불립니다.
			
			전통적으로 입력 데이터는 $D$가 feature 수인 $N × D$의 설계 행렬 $X$로 저장됩니다. 벡터 공간 모델의 맥락에서는 입력 데이터를 $D × N$의 용어 빈도(term frequency) 행렬로 나타내는 것이 더 일반적이며, 여기서 $TF_{ij}$는 문서 $j$에서 용어 $i$의 빈도를 나타냅니다. 
			
			![[Pasted image 20240105003312.png]]
			위의 그림에서 이를 시각적으로 확인할 수 있습니다. TF-IDF(Term Frequency-Inverse Document Frequency) 값으로 원시 카운트가 대체된 용어-문서 행렬의 예시입니다. 더 어두운 셀은 더 큰 값입니다.
	- #### TF-IDF
		- **문제상황:**
			  문서를 단어 카운트 벡터로 나타내는 문제 중 하나는 빈번한 단어가 의미적 내용이 많지 않더라도 그 단어 카운트의 크기가 더 크기 때문에 과도한 영향을 미칠 수 있다는 것입니다.
			  
		- **해결방안:**
			 이를 해결하기 위한 일반적인 방법 중 하나는 로그를 취하여 카운트의 영향을 줄이는 것입니다. 특정 문서 내에서 많이 발생하는 단어의 영향을 줄이기 위해 이러한 변환을 수행합니다.
			  
			 일반적으로(모든 문서를 대상으로) 많이 발생하는 단어의 영향을 줄이기 위해 역문서 빈도(IDF)라는 양을 계산합니다. $IDF_i$는 다음과 같이 정의됩니다: 
			 
			 $IDF_i = log(\frac{N}{(1 + DF_i)})$
			 여기서 $DF_i$는 용어 $i$가 포함된 문서의 수입니다. 이러한 변환을 결합하여 TF-IDF 행렬을 계산할 수 있습니다:
			 
			 $TFIDF_{ij}=log(TF_{ij}+1)\times IDF_{i}$
			 이는 문서의 더 의미 있는 표현을 제공하며 많은 머신 러닝 알고리즘의 입력으로 사용될 수 있습니다.
			 
	- #### Word embeddings
		- **문제정의:**
			  TF-IDF 변환은 "유의미한" 단어에 더 많은 가중치를 두고 "유의미하지 않은" 단어에 덜 가중치를 두어 원시 카운트 벡터에 비해 성능을 향상시킵니다. 그러나 이는 카운트 벡터가 유도된 원핫 인코딩의 근본적인 문제를 해결하지 않습니다. 이 문제는 벡터 공간에서 "man"과 "woman"과 같은 의미론적으로 유사한 단어가 "man"과 "banana"와 같은 의미론적으로 다른 단어보다 가깝지 않을 수 있다는 것입니다. 따라서 대부분의 예측 모델에서 암시적으로 제시되는 입력 공간에서 가까운 지점이 유사한 출력을 가져야 한다는 가정은 부적절합니다.
			  
		- **해결방안:**
			이 문제를 해결하는 표준 방법은 각 희소한 원핫 벡터 $x_{nt}​∈{\{0,1\}}^V$를 낮은 차원의 밀집 벡터 $e_{nt}​∈R^K$로 매핑하는 워드 임베딩을 사용하는 것입니다. 여기서 $E∈R^{K×V}$이 학습되어 의미적으로 유사한 단어가 가까이 배치되도록 합니다. 이러한 임베딩을 학습하는 여러 가지 방법이 있습니다.
			
			임베딩 행렬을 얻으면 변수 길이의 텍스트 문서를 단어 임베딩의 가방으로 나타낼 수 있습니다. 그런 다음 임베딩을 합산(또는 평균)하여 고정 길이 벡터로 변환할 수 있습니다.
			
			$\bar{e}_{n}​=\sum_{t=1}^{T}{​e_{nt}​}=E\tilde{x}_n​$
			
			여기서 $\tilde{x}_n$​은 위의 식의 단어 가방 표현입니다. 그런 다음 이를 로지스틱 회귀 분류기 내부에서 사용할 수 있으며, 이는 간단히 소개한 모델 형식을 가지고 있습니다.
			
			$p(y=c∣x_n​,θ)=softmaxc​(WE\tilde{x}_n​)$
			
			우리는 종종 사전 훈련된 단어 임베딩 행렬 E를 사용하며, 이 경우 모델은 W에 대한 단순화된 매개변수 추정을 제공합니다.
			
	- #### Dealing with novel words
		- **문제정의:**
			  시험 시간에 모델은 이전에 본 적이 없는 완전히 새로운 단어를 만날 수 있습니다. 이것은 색인어 없음(out of vocabulary 또는 OOV) 문제로 알려져 있습니다. 이런 새로운 단어들은 열린 클래스인 단어의 집합 때문에 발생할 수밖에 없습니다. 예를 들어, 고유 명사(사람들과 장소의 이름)의 집합은 무제한입니다.
			  
		- **해결방안:**
			  이 문제를 해결하기 위한 표준 휴리스틱은 모든 새로운 단어를 "unknown"을 나타내는 특수 기호 UNK로 대체하는 것입니다. 그러나 이는 정보를 잃게 됩니다. 예를 들어, "athazagoraphobia"라는 단어를 만나게 되면, 이 단어가 "뭔가를 두려워하는 것"을 의미한다고 추측할 수 있습니다. 왜냐하면 영어에서 (그리스어에서 파생된) "두려움"을 의미하는 흔한 접미사로 "phobia"가 사용되기 때문입니다.
			  
			문자 수준에서 작업할 수 있지만, 이는 모델이 공통된 문자 조합을 단어로 그룹화하는 방법을 학습해야 한다는 것을 의미합니다.
			
- ### Handling missing data
	- **문제정의:**
		가끔 데이터가 누락될 수 있습니다. 여기서 입력 x나 출력 y의 일부가 알려지지 않을 수 있습니다. 출력이 훈련 중에 알려지지 않으면 해당 예제는 라벨이 없습니다. 따라서 훈련 또는 테스트 시간 또는 둘 다에서 일부 입력 기능이 누락될 수 있는 경우에 중점을 둡니다.
		
	- **해결방안:**
		  누락된 데이터에 대한 모델링을 위해 이진 변수 행렬 M을 도입하고, 이를 사용하여 누락된 부분과 가시적인 부분을 나누어 설명합니다. 이 누락 메커니즘은 무작위로 발생할 수 있으며(MCAR 또는 MAR) 때로는 숨겨진 기능에 의존할 수 있습니다(NMAR). 이 책에서는 주로 MAR 가정을 사용하며, 누락된 값은 평균 값으로 대체하거나 생성 모델을 사용하여 채울 수 있다고 설명하고 있습니다. NMAR 경우에는 누락된 데이터 메커니즘을 모델링해야 합니다.
## Discussion
