...
- **함수 설명:**
	...
	- `wide` 함수는 "Wide" 모델을 훈련하고 평가하는 역할을 합니다.
	- 주요 매개변수로는 훈련 및 테스트 데이터셋, wide 모델에 사용될 열들(`wide_cols`), 교차될 열들(`x_cols`), 예측 대상 열(`target`), 모델의 타입(`model_type`), 피팅 방법(`method`) 등이 있습니다.
	- `model_type`이 'wide'인 경우 모델을 훈련하고 결과를 출력하며, 'wide_deep'인 경우 모델의 입력 데이터를 반환합니다.
...
- **데이터 전처리:**
	...
	- 'IS_TRAIN'이라는 새로운 열을 만들어 훈련 및 테스트 데이터를 구분합니다.
...
- **Wide 모델 구성 및 훈련:**
	...
	- `fit_param` 딕셔너리에서 적절한 활성화 함수, 손실 함수, 메트릭을 가져옵니다.
		 `fit_param['logistic'] = ('sigmoid', 'binary_crossentropy', 'accuracy')`
	- 'wide' 모델의 입력 레이어를 정의하고, Dense 레이어를 사용하여 출력 레이어를 연결합니다. 이는 선형 모델의 역할을 합니다.
